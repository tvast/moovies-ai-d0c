...
import { createBuildApp, createDevApp, createPage, preparePageChunk, prepareRoutes } from "@vuepress/core";
...
const watchPageFiles = (app) => {
	...
	for (const pattern of app.options.pagePatterns) if (pattern.startsWith("!")) ignorePatterns.push(pattern.slice(1));
	else pagePatterns.push(pattern);
	...
	const pagesWatcher = chokidar.watch(".", {
		...
		ignored: (filepath, stats) => {
			...
			if (stats?.isDirectory()) return filepath === tempDir || filepath === cacheDir;
			return !!stats?.isFile() && !pageMatcher(path.relative(sourceDir, filepath));
		},
		ignoreInitial: true
	});
...
//#endregion
export { build, cli, createPageDepsHelper, defineUserConfig, dev, handlePageAdd, handlePageChange, handlePageUnlink, info, loadUserConfig, resolveAppConfig, resolveCliAppConfig, resolveConfig, resolveUserConfig, resolveUserConfigConventionalPath, resolveUserConfigPath, transformUserConfigToPlugin, watchPageFiles, watchUserConfigFile };
